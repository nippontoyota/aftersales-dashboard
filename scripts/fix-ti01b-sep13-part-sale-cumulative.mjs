// TI01B's "13 Sep" Part Sale upload is a full month-to-date cumulative
// export (730 distinct BillNo, spanning SaleDate 1-12 Sep) — their
// well-documented standing pattern for this report type (see
// docs/data-reconciliation.md's earlier TI01B fixes). It's a clean
// superset of both the existing 10 Sep (602/602 BillNo present) and
// 11 Sep (666/666 BillNo present) snapshots. Left alone, both would
// double-count into MTD.
//
// Deletes the now-redundant 10 Sep and 11 Sep snapshots + raw rows,
// leaving 13 Sep as the sole (already-cumulative) holder.
//
//   node scripts/fix-ti01b-sep13-part-sale-cumulative.mjs [--commit]
...
try {
  const { rows: before } = await client.query(
    `select date::text, source_file_name, engine_flush, injector_cleaner, synthetic_oil_ltrs, brake_cleaning_spray, external_sales
       from part_sale_snapshots where branch=$1 and date >= '2026-09-01' order by date`,
    [BRANCH]
  );
  console.log("Before:", before);
  const { rows: mtdBefore } = await client.query(
    `select sum(engine_flush) ef, sum(injector_cleaner) ic, sum(synthetic_oil_ltrs) so, sum(brake_cleaning_spray) bcs, sum(external_sales) es
