...
export default async function VpBranchesPage({
  ...
  searchParams: Promise<{ date?: string; branch?: string; flag?: string }>;
}) {
  const admin = await requireVpAccess();
  return (
    <AppShell current="vp-branches" showDashboardLink vpNav identity={adminIdentityLabel(admin)}>
...
async function Branches({ searchParams }: { searchParams: Promise<{ date?: string; branch?: string; flag?: string }> }) {
  const params = await searchParams;
  const data = await loadVpData(params.date);
  if (!data) {
    return (
      <div className="mx-auto w-full max-w-3xl px-6 py-8">
        <VpHeader eyebrow="Nippon Group · Service" title="Branches" flagHref="/vp/branches?flag=1" />
  ...
  const branch = selected ? data.report.branches.find((b) => b.branch === selected)! : null;
  const region = branch ? regionForBranch(branch.branch) : null;
  const flagBase = `/vp/branches?date=${data.date}${selected ? `&branch=${selected}` : ""}`;

  return (
    <div className="mx-auto max-w-[1200px] px-6 py-8">
      <VpHeader
        eyebrow="Nippon Group · Service"
        title={branch ? branch.branch : "Branches"}
        subtitle={
          branch
            ? `${region ?? "—"} region · month-to-date, with a run-rate projection to month-end.`
            : "Ranked by total revenue this month — pick one for its full figures."
        }
        dates={data.dates}
        date={data.date}
        basePath="/vp/branches"
        dateExtraParams={selected ? { branch: selected } : undefined}
        flagHref={`${flagBase}${branch ? `&fbranch=${branch.branch}` : ""}&flag=1`}
      >
