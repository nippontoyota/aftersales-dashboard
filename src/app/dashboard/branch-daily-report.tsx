...
function Cell({ value, fmt, className = "" }: { value: number | null | undefined; fmt: NumFormatter; className?: string }) {
  return (
    <td className={`whitespace-nowrap py-1.5 pl-4 text-right tabular-nums ${className}`}>
      {value === undefined || value === null ? <span className="text-slate-300">—</span> : fmt(value)}
    </td>
  );
}
            ...
            {rows.map((row, i) => {
              if (row.kind === "group") {
                return (
                  <tr key={`g-${i}`}>
                    <td
                      colSpan={5}
                      className="border-t border-slate-200 bg-slate-50 px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500"
                    >
                      {row.label}
                    </td>
                  </tr>
              ...
              return (
                <tr key={`m-${i}`} className="border-t border-slate-100">
                  <td className={`whitespace-nowrap py-1.5 pl-4 pr-3 ${row.strong ? "font-semibold text-slate-900" : "text-slate-600"}`}>
                    {row.label}
                  </td>
                  <Cell value={row.today} fmt={row.fmt} className="text-slate-500" />
                  <Cell value={row.mtd} fmt={row.fmt} className={row.strong ? "font-semibold text-slate-900" : "text-slate-900"} />
                  <Cell value={row.target} fmt={row.fmt} className="text-slate-500" />
                  <td className={`whitespace-nowrap py-1.5 pl-4 pr-4 text-right font-medium tabular-nums ${ratio == null ? "text-slate-300" : TONE_TEXT[tone]}`}>
