  ...
  return (
    <div className="mb-4 flex items-start gap-3 rounded-xl border border-warn/30 bg-warn-soft/40 px-4 py-3 shadow-sm print:hidden">
      ...
      <p className="text-[13px] leading-relaxed text-fg">
        <span className="font-semibold tracking-wide text-warn">DRAFT (Unpublished):</span> Data for this date is actively syncing and incomplete. Only <span className="font-semibold tabular-nums">{uploadedBranches} / {totalBranches}</span> branches have uploaded their figures so far.
      </p>
    </div>
  );
}
