  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-200 bg-white">
      <button
        ...
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex w-full items-center justify-between gap-2 px-3.5 py-2.5 text-left hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-inset"
      >
        <span className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
          <span className="text-xs font-semibold uppercase tracking-wide text-slate-500">{title}</span>
          {subtitle ? <span className="text-[11px] font-normal normal-case text-slate-400">{subtitle}</span> : null}
        </span>
        <svg
